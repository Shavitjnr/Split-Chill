...
build_backend() {
    ...
    CGO_ENABLED=1 go build -a -v -trimpath -ldflags "-w -s -linkmode external -extldflags '-static' $backend_build_extra_arguments" -o splitchill-ai splitchill-ai.go
    chmod +x splitchill-ai
}
...
build_package() {
    ...
    package_file_name="splitchill-ai-$package_file_name-$(arch).tar.gz"
...
build_docker() {
    ...
    docker_tag="splitchill-ai:$docker_tag"
